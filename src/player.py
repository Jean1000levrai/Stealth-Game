import easyPyEngine.easyPyEngine as epe
from var import *
from platform import Platform

class Player:
    def __init__(self, engine, x, y, speed=200):
        self.engine = engine
        self.x = x
        self.y = y
        self.speed = speed

        # Load all sprites
        self.sprites = {}
        for direction, frames in player_img.items():
            self.sprites[direction] = {}
            for key, path in frames.items():
                self.sprites[direction][key] = epe.Sprite(engine, path, 100, 100)

        self.direction = "right"
        self.frame_counter = 0
        self.frame_delay = 0.1
        self.moving = False
        self.attacking = False
        self.attack_duration = 0.3
        self.attack_timer = 0

        self.width = 100
        self.height = 100

        # Gravity
        self.vel_y = 0
        self.gravity = 800  # pixels per second squared
        self.ground_y = 500 - self.height  # ground level (based on your green ground)

    def update_player(self, dt, mobs=None, platforms=None):
        self.moving = False

        # Horizontal movement
        if self.engine.is_key_pressed("a"):
            self.x -= self.speed * dt
            self.direction = "left"
            self.moving = True
        if self.engine.is_key_pressed("d"):
            self.x += self.speed * dt
            self.direction = "right"
            self.moving = True

        # Attack input
        if self.engine.is_key_pressed("space") and not self.attacking:
            self.attacking = True
            self.attack_timer = 0

        if self.attacking:
            self.attack_timer += dt
            if self.attack_timer >= self.attack_duration:
                self.attacking = False

        # Platform collision
        on_platform = False
        if platforms:
            for platform in platforms:
                if (
                    self.x + self.width > platform.x and
                    self.x < platform.x + platform.width and
                    self.y + self.height <= platform.y + 10 and  # small tolerance
                    self.y + self.height >= platform.y - 5 and
                    self.engine.is_key_pressed("e")
                ):
                    self.y = platform.y - self.height
                    on_platform = True
                    break

        # Gravity (fall if not on platform)
        if not on_platform:
            self.vel_y += self.gravity * dt
            self.y += self.vel_y * dt
        else:
            self.vel_y = 0

        # Ground collision
        if self.y > self.ground_y:
            self.y = self.ground_y
            self.vel_y = 0

        # Attack collision with mobs
        if self.attacking and mobs:
            for mob in mobs[:]:
                if self._collides_with(mob):
                    mobs.remove(mob)

        # Walk animation
        if self.moving:
            self.frame_counter += dt
        else:
            self.frame_counter = 0

    def draw_player(self):
        if self.attacking:
            sprite_to_draw = self.sprites[self.direction]["atk"]
        elif self.moving:
            frames = ["walk1", "walk2", "walk3"]
            index = int(self.frame_counter / self.frame_delay) % len(frames)
            sprite_to_draw = self.sprites[self.direction][frames[index]]
        else:
            sprite_to_draw = self.sprites[self.direction]["idle"]

        sprite_to_draw.draw(self.engine, self.x, self.y)

    def _collides_with(self, mob):
        return (
            self.x < mob.x + mob.width and
            self.x + self.width > mob.x and
            self.y < mob.y + mob.height and
            self.y + self.height > mob.y
        )